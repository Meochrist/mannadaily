...
export default async function ProfilePage() {
  ...
  try {
    const user = await query("SELECT id, name, email, image, createdAt, favoriteMascot, meditationProgress FROM users WHERE id = $1", [userId]);
    ...
    const sessions = await query("SELECT id, type, xpEarned, createdAt FROM daily_sessions WHERE userId = $1 ORDER BY createdAt DESC LIMIT 10", [userId]);
    const verseNotes = await query("SELECT id, content, isVoice, createdAt, book, chapter, verse FROM verse_notes WHERE userId = $1 ORDER BY createdAt DESC", [userId]);
    const highlights = await query("SELECT id, color, book, chapter, verse, text, translation, createdAt FROM highlights WHERE userId = $1 ORDER BY createdAt DESC", [userId]);
    const userBadges = await query("SELECT ub.badgeId, ub.earnedAt FROM user_badges ub WHERE ub.userId = $1", [userId]);
    const allBadges = await query("SELECT * FROM badges");
    const readingPlans = await query(`
      SELECT rpe.id, rpe.planId, rpe.currentDay, rpe.completed, rp.name, rp.duration
      FROM reading_plan_enrollments rpe
      JOIN reading_plans rp ON rpe.planId = rp.id
      WHERE rpe.userId = $1
    `, [userId]);
    const readingProgress = await query("SELECT planId, COUNT(*) as count FROM reading_plan_progress WHERE userId = $1 GROUP BY planId", [userId]);

    const initials = userData.name 
      ? userData.name.split(" ").map((n: string) => n[0]).join("").toUpperCase().slice(0, 2) 
      : "U";
    
    const favoriteMascot = userData.favoriteMascot || "manny";
    const MascotComponent = mascotMap[favoriteMascot] || Manny;
    const levelName = progress[0]?.level || "Semence";
    const levelClass = levelStyles[levelName] || "bg-indigo-50 text-indigo-700 border-indigo-200";

    const totalXP = progress[0]?.totalXP ?? 0;
    const sessionsCount = progress[0]?.sessionsTotal ?? sessions.length;
    const currentStreak = streak[0]?.currentStreak ?? 0;
    const longestStreak = streak[0]?.longestStreak ?? 0;
    const versesLearned = progress[0]?.versesLearned ?? 0;
    
    const activePlans = readingPlans.filter((p: any) => !p.completed);
    const completedPlans = readingPlans.filter((p: any) => p.completed);
      ...
      <div className="max-w-6xl mx-auto p-4 space-y-8 pb-24">
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 sm:p-8 flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left z-10">
            {userData.image ? (
              <img src={userData.image} alt={userData.name || "Profil"} className="w-20 h-20 rounded-full object-cover border-2 border-indigo-100 shadow-sm" />
            ) : (
              <div className="w-20 h-20 bg-gradient-to-tr from-indigo-500 to-purple-600 rounded-full flex items-center justify-center text-white text-2xl font-black shadow-md">
                {initials}
              </div>
            )}
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3 text-xs text-slate-500 font-medium">
                <span className="flex items-center gap-1"><Mail className="w-3.5 h-3.5 text-slate-400" />{userData.email}</span>
                <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5 text-slate-400" />Inscrit le {new Date(userData.createdAt).toLocaleDateString("fr-FR", { year: "numeric", month: "long", day: "numeric" })}</span>
              </div>
              <div className="flex justify-center sm:justify-start pt-1">
                <span className={cn("text-[10px] font-black border px-3 py-1 rounded-full uppercase tracking-wider shadow-sm flex items-center gap-1.5", levelClass)}>
                  <Award className="w-3.5 h-3.5" />Niveau : {levelName}
                </span>
              </div>
            </div>
          </div>
          <div className="flex flex-col items-center gap-1.5 z-10 flex-shrink-0 bg-slate-50/55 border border-slate-100 rounded-2xl p-4 min-w-[140px]">
            ...
            <span className="text-[10px] text-indigo-600 font-black uppercase tracking-wider block">Compagnon favori</span>
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{favoriteMascot}</span>
          </div>
        </div>
        ...
        <ShareCard type="streak" streakValue={currentStreak} levelName={levelName} title="Partage ton profil et ton streak de méditation biblique quotidien avec tes proches !" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-8">
            <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm space-y-4">
              <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider flex items-center gap-1.5 border-b border-slate-50 pb-3">
                <Calendar className="w-4 h-4 text-indigo-600" />Mes Plans de Lecture
              </h3>
              {readingPlans.length === 0 ? (
                ...
                <div className="space-y-4">
                  {readingPlans.map((enrollment: any) => {
                    const completedDaysCount = readingProgress.find((p: any) => p.planId === enrollment.planId)?.count || 0;
                    const percent = Math.min(100, Math.round((completedDaysCount / enrollment.duration) * 100));
                    return (
                      <div key={enrollment.id} className="border border-slate-100 rounded-2xl p-4 space-y-3">
                        <div className="flex justify-between items-start gap-2">
                          <div>
                            <span className={cn("text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider", enrollment.completed ? "bg-emerald-50 text-emerald-700" : "bg-indigo-50 text-indigo-700")}>
                              {enrollment.completed ? "Complété" : "En cours"}
                            </span>
                            <h4 className="text-xs font-black text-slate-800 mt-1">{enrollment.name}</h4>
                          </div>
                          <span className="text-[10px] text-slate-400 font-bold">Jour {enrollment.currentDay} / {enrollment.duration}</span>
                        </div>
                        <div className="space-y-1.5">
                          <div className="flex justify-between text-[9px] font-bold text-slate-400 uppercase"><span>Progression</span><span>{percent}%</span></div>
                          <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden border border-slate-200/50">
                            <div className={cn("h-full rounded-full transition-all", enrollment.completed ? "bg-emerald-500" : "bg-indigo-600")} style={{ width: `${percent}%` }} />
                          </div>
                        </div>
                      </div>
                    );
            ...
            <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm space-y-4">
              <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider flex items-center gap-1.5 border-b border-slate-50 pb-3">
                <FileText className="w-4 h-4 text-indigo-600" />Mon Journal Spirituel
              </h3>
              {verseNotes.length === 0 ? (
                ...
                <div className="space-y-4 max-h-[480px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-slate-200">
                  {verseNotes.map((note: any) => (
                    <div key={note.id} className="bg-slate-50/50 border border-slate-100 rounded-2xl p-4 space-y-3">
                      <div className="flex justify-between items-center">
                        <span className="text-[10px] font-black text-indigo-600 bg-indigo-50/80 px-2.5 py-0.5 rounded-lg">{note.book} {note.chapter}:{note.verse}</span>
                      ...
                      <div className="flex justify-end pt-1">
                        <Link href={`/bible?book=${encodeURIComponent(note.book)}&chapter=${note.chapter}&verse=${note.verse}`} className="text-[9px] font-black text-indigo-600 hover:text-indigo-800 uppercase tracking-widest flex items-center gap-0.5 hover:underline">
                          Consulter dans la Bible <ChevronRight className="w-3 h-3" />
                        </Link>
                      </div>
                    </div>
                  ))}
            ...
            <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm space-y-4">
              <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider flex items-center gap-1.5 border-b border-slate-50 pb-3">
                <Highlighter className="w-4 h-4 text-indigo-600" />Mes Versets Surlignés
              </h3>
              {highlights.length === 0 ? (
                ...
                <div className="space-y-3.5 max-h-[400px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-slate-200">
                  {highlights.map((h: any) => (
                    <div key={h.id} className={cn("p-4 rounded-2xl flex flex-col justify-between gap-2.5", highlightColors[h.color] || highlightColors.yellow)}>
                      <p className="text-xs leading-relaxed font-semibold italic">« {h.text} »</p>
                      <div className="flex justify-between items-center text-[9px] font-bold">
                        <span className="text-slate-500 font-extrabold uppercase tracking-wide">{h.book} {h.chapter}:{h.verse} ({h.translation || 'LSG'})</span>
                        <Link href={`/bible?book=${encodeURIComponent(h.book)}&chapter=${h.chapter}&verse=${h.verse}`} className="text-indigo-600 hover:text-indigo-800 uppercase tracking-widest flex items-center gap-0.5">Lire le chapitre <ChevronRight className="w-3 h-3" /></Link>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm space-y-4">
              <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider flex items-center gap-1.5 border-b border-slate-50 pb-3">
                <Trophy className="w-4 h-4 text-indigo-600" />Mes Badges
              </h3>
              <div className="grid grid-cols-2 gap-3.5">
                {allBadges.map((badge: any) => {
                  const userBadge = userBadges.find((ub: any) => ub.badgeId === badge.id);
                  return <BadgeCard key={badge.id} name={badge.name} description={badge.description} icon={badge.icon} earnedAt={userBadge ? userBadge.earnedAt : null} />;
                })}
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm space-y-4">
              <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider flex items-center gap-1.5 border-b border-slate-50 pb-3">
                <History className="w-4 h-4 text-indigo-600" />Dernières Sessions
              </h3>
              {sessions.length === 0 ? (
                ...
                <div className="space-y-3">
                  {sessions.map((session: any) => (
                    <div key={session.id} className="border-b border-slate-50 pb-3 last:border-b-0 last:pb-0 flex justify-between items-center gap-2">
                      ...
                      <span className="text-xs font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-lg flex-shrink-0">+{session.xpEarned} XP</span>
                    </div>
                  ))}
                </div>
              )}
