...
// Dictionnaire des composantes de mascottes
...
export default async function ProfilePage() {
  ...
  try {
    ...
    const user = db.prepare(`
      SELECT id, name, email, image, createdAt, favoriteMascot, meditationProgress
      FROM users WHERE id = ?
    `).get(userId) as any;
    ...
    const sessions = db.prepare(`
      SELECT id, type, xpEarned, createdAt FROM daily_sessions 
      WHERE userId = ? ORDER BY createdAt DESC LIMIT 10
    `).all(userId) as any[];

    // Récupérer les notes de versets
    const verseNotes = db.prepare(`
      SELECT id, content, isVoice, createdAt, book, chapter, verse 
      FROM verse_notes WHERE userId = ? ORDER BY createdAt DESC
    `).all(userId) as any[];

    // Récupérer les highlights
    const highlights = db.prepare(`
      SELECT id, color, book, chapter, verse, text, translation, createdAt 
      FROM highlights WHERE userId = ? ORDER BY createdAt DESC
    `).all(userId) as any[];

    // Récupérer les badges utilisateur
    const userBadges = db.prepare(`
      SELECT ub.badgeId, ub.earnedAt FROM user_badges ub WHERE ub.userId = ?
    `).all(userId) as any[];
    ...
    // Récupérer la progression des plans
    const readingProgress = db.prepare(`
      SELECT planId, COUNT(*) as count FROM reading_plan_progress WHERE userId = ? GROUP BY planId
    `).all(userId) as any[];
    ...
    const sessionsCount = progress?.sessionsTotal ?? sessions.length;
    const currentStreak = streak?.currentStreak ?? 0;
    const longestStreak = streak?.longestStreak ?? 0;
    const versesLearned = progress?.versesLearned ?? 0;
                      ...
                      <div key={enrollment.id} className="border border-slate-100 rounded-2xl p-4 space-y-3">
                        <div className="flex justify-between items-start gap-2">
                          ...
                          <span className="text-[10px] text-slate-400 font-bold">
                            Jour {enrollment.currentDay} / {enrollment.duration}
                          </span>
                        </div>
                ...
                <div className="space-y-4 max-h-[480px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-slate-200">
                  {verseNotes.map((note: any) => (
                    <div key={note.id} className="bg-slate-50/50 border border-slate-100 rounded-2xl p-4 space-y-3">
                      <div className="flex justify-between items-center">
                        <span className="text-[10px] font-black text-indigo-600 bg-indigo-50/80 px-2.5 py-0.5 rounded-lg">
                      ...
                      <div className="flex justify-end pt-1">
                        <Link 
                          href={`/bible?book=${encodeURIComponent(note.book)}&chapter=${note.chapter}&verse=${note.verse}`}
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center text-[9px] font-bold">
                        <span className="text-slate-500 font-extrabold uppercase tracking-wide">
                          {h.book} {h.chapter}:{h.verse} ({h.translation || 'LSG'})
                        </span>
                        <Link 
                          href={`/bible?book=${encodeURIComponent(h.book)}&chapter=${h.chapter}&verse=${h.verse}`}
                          className="text-indigo-600 hover:text-indigo-800 uppercase tracking-widest flex items-center gap-0.5"
                        >
