...
export default function XPBar({ currentXP, levelName, progressPercent }: XPBarProps) {
  return (
    <div className="w-full bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm transition-colors">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-xs text-slate-400 dark:text-slate-500 font-semibold uppercase tracking-wider">Niveau actuel</p>
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">{levelName}</h3>
          </div>
        </div>
        <span className="text-sm font-bold text-amber-650 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 px-3 py-1 rounded-full">
          {currentXP} XP
        </span>
      </div>
