...
function fetchChapter(version: string, bookNr: number, chapterNr: number, retries = 3): Promise<GetBibleChapter> {
  const url = `https://api.getbible.net/v2/${version}/${bookNr}/${chapterNr}.json`;
  return new Promise((resolve, reject) => {
    const request = () => {
      https.get(url, (res) => {
        if (res.statusCode !== 200) {
...
async function seedVersion(key: string, name: string) {
  ...
  for (let bookNr = 1; bookNr <= 66; bookNr++) {
    ...
    process.stdout.write(`📖 Livre ${bookNr}/66 — ${bookName}... `);
    ...
    for (let c = 0; c < chapterNumbers.length; c += chunkSize) {
      ...
      const results = await Promise.allSettled(
        chunk.map(chapterNr => fetchChapter(key, bookNr, chapterNr))
      );
  ...
  console.log(`=== Importation ${name} terminée ! ===`);
  console.log(`📊 Livres : 66 | Chapitres : ${totalChaptersProcessed} | Versets : ${totalVersesInserted}`);
  if (errors.length > 0) {
...
async function main() {
  console.log("=== Détection des versions françaises disponibles sur getbible.net ===");
  const translationsRaw = await fetchText("https://api.getbible.net/v2/translations.json");
  ...
  if (versionsToSeed.length === 0) {
    console.log("✅ Toutes les versions françaises disponibles de getbible.net (LSG, Darby, Martin) sont déjà enregistrées.");
    return;
  }

  console.log(`Nouvelles versions détectées : ${versionsToSeed.map(v => v.name).join(", ")}`);
...
main()
  .catch((e) => {
    console.error("Erreur durant le seed global :", e);
    process.exit(1);
  })
  .finally(async () => {
