...
function fetchChapter(bookNr: number, chapterNr: number, retries = 3): Promise<GetBibleChapter> {
  const url = `https://api.getbible.net/v2/darby/${bookNr}/${chapterNr}.json`;
...
async function main() {
  console.log("=== Début du Seed de la Bible Darby (api.getbible.net/v2/darby) ===");
  ...
  for (let bookNr = 1; bookNr <= 66; bookNr++) {
    ...
    // Charger tous les chapitres du livre en parallèle (tranches de 20 max pour ne pas spammer)
  ...
  console.log("\n========================================");
  console.log(`=== Seed de la Bible Darby terminé ! ===`);
...
main()
  .catch((e) => {
    console.error("Erreur critique durant le seed Darby :", e);
    process.exit(1);
  })
  .finally(async () => {
